...
/**
  * This provides an event sourced behavior. It has a state, [[MicroserviceAuditState]], which
  ...
  */
object MicroserviceAuditBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[MicroserviceAuditCommand]): Behavior[MicroserviceAuditCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, MicroserviceAuditEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[MicroserviceAuditCommand, MicroserviceAuditEvent, MicroserviceAuditState](
        persistenceId = persistenceId,
        emptyState = MicroserviceAuditState.initial,
...
/**
  ...
  */
case class MicroserviceAuditState(message: String, timestamp: String) {
  def applyCommand(cmd: MicroserviceAuditCommand): ReplyEffect[MicroserviceAuditEvent, MicroserviceAuditState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: MicroserviceAuditEvent): MicroserviceAuditState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[MicroserviceAuditEvent, MicroserviceAuditState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[MicroserviceAuditEvent, MicroserviceAuditState] =
    Effect
...
object MicroserviceAuditState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: MicroserviceAuditState = MicroserviceAuditState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[MicroserviceAuditCommand]("MicroserviceAuditAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[MicroserviceAuditState] = Json.format
}

/**
  * This interface defines all the events that the MicroserviceAuditAggregate supports.
  */
sealed trait MicroserviceAuditEvent extends AggregateEvent[MicroserviceAuditEvent] {
  def aggregateTag: AggregateEventTag[MicroserviceAuditEvent] = MicroserviceAuditEvent.Tag
}

object MicroserviceAuditEvent {
  val Tag: AggregateEventTag[MicroserviceAuditEvent] = AggregateEventTag[MicroserviceAuditEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MicroserviceAuditEvent
...
/**
  ...
  */
trait MicroserviceAuditCommandSerializable

/**
  * This interface defines all the commands that the MicroserviceAuditAggregate supports.
  */
sealed trait MicroserviceAuditCommand
    extends MicroserviceAuditCommandSerializable
...
case class UseGreetingMessage(message: String, replyTo: ActorRef[Confirmation])
    extends MicroserviceAuditCommand
...
/**
  ...
  * application loader.
  */
object MicroserviceAuditSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MicroserviceAuditState],
